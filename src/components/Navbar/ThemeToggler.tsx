...
const ThemeToggler = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={`dropdown dropdown-end cursor-pointer`}>
      ...
      >
        <IconSun />
        {className ? null : <IconChevronDown size={16} />}
      </div>
      <ul
        tabIndex={0}
        className="menu dropdown-content bg-base-300/80 rounded-box z-[1] w-52 p-2 overflow-y-auto block h-80 shadow-2xl text-base-content"
      >
        {themes.map((theme) => (
          <li key={theme}>
            <input
